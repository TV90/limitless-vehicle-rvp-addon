...
public final class RVP_DeploymentMotionUtil {
    ...
    /** 小于该绝对值的部署纵向速度直接归零，避免无限保留浮点残量。 */
    private static final double ZERO_VERTICAL_SPEED = 1.0E-6D;
    ...
    /**
     * 按半衰期衰减一次部署纵向速度；只消费 Y，返回值的 X/Z 恒为 0。
     *
     * @param verticalVelocity 当前部署纵向速度，单位格/Tick
     * @param halfLifeTicks 半衰期，单位 Tick；非正或非有限值表示不衰减
     * @return 下一 Tick 的部署纵向速度
     */
    public static Vec3 decayVertical(Vec3 verticalVelocity, float halfLifeTicks) {
        if (verticalVelocity == null) {
            return Vec3.ZERO;
        }
        double vertical = verticalVelocity.y;
        if (!Float.isFinite(halfLifeTicks) || halfLifeTicks <= 0f) {
            return new Vec3(0.0D, vertical, 0.0D);
        }
        double next = vertical * Math.pow(0.5D, 1.0D / halfLifeTicks);
        return Math.abs(next) <= ZERO_VERTICAL_SPEED ? Vec3.ZERO : new Vec3(0.0D, next, 0.0D);
    }
     ...
     * @param previousComposedVelocity 上一 Tick 四分量合成的总速度
