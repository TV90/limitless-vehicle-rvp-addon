...
**当前验收状态：载具消失、分辨率切换、remote-only、关闭 diagnostics 已经用户实机确认通过；观瞄近距目标变黑的取光修复也已通过实机验收。**

当前实现已经把 RVP 载具从单一的 Forge 实体/`AFTER_ENTITIES` 绘制，扩展为可选的 Distant Horizons（下文简称 DH）优先显示与深度感知合成通道：
...
**2026-09-04 新增无 DH 高度策略：** 服务端 `ywzj_rvp-common.toml` 的 `[remoteVehicleRendering]` 中新增 `minHeightAboveGroundWithoutDH = 25`（整数，单位米/格，`-1` 禁用）。阈值通过协议 `7` 的完整载具快照下发：客户端未安装 DH 或关闭 DH 官方地形渲染时，低于阈值的远距代理在预算与预热前隐藏，等于阈值仍显示。开启 DH 后不执行该过滤；RVP `compatMode=OFF` 不等同于 DH 自身关闭。原生追踪实体与真实载具保护层不受影响。详见[载具超远距离渲染指南 §2.1](./RVP_载具实体超远距离渲染技术文档与调参指南_20260818.md#21-未开启-dh-时的最低离地高度2026-09-04)。
...
- 真实载具保护层通过正常实体调度器 `getPackedLightCoords(vehicle, partialTick)` 冻结光照，间接消费本体 `getLightProbePosition`；禁止用 `vehicle.blockPosition()` 替代，否则接地原点可能采到地块内部的零天空光。摧毁暗化在取得正常光照后只应用一次；不固定加高、不设亮度下限。
...
- 履带、尾焰和带独立动画的饰品不会在保护层重复调用本体 `render()`；这些方法会推进自身动画，当前保护层仅覆盖主体几何。
...
### 移动屏幕准星时整车消失：高模回退预算不足

现场共有 X 个需要高模回退的载具，原先 maxFallbackHighModels 小于该数量。需调高该参数；视角改变会重新排序候选对象，导致部分载具被移出预算，看起来像“准星移动后整车消失”。
