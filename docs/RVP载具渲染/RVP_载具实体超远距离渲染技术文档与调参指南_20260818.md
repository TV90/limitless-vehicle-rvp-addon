...
> 文档日期：2026-08-18  
> 追加更新：2026-09-24（补充本体雷达广播克隆与 RVP HUD 显示插值的边界）
> 代码基线：远裁剪墙修复后的激进 Billboard LOD 实现，含动态快照观察方向、纹理 U/V 轴修复、无 DH 高度策略及类型化部件终态（网络协议版本 14）
...
| 当前实现 | 激进 Billboard LOD | 服务端权威策略、动态高模快照、视角分桶 LRU、预热降级、离屏观察方向/纹理原点修复、类型化部件终态和协议版本 14。 |

代码、自动化测试和构建层面的实现已经完成；重构计划中 RVR-16～RVR-23 的真实游戏验收仍需按场景执行，本文第 13 节给出了对应的回归清单。

## 2. 快速启用

默认配置已经启用本功能。服务端和客户端版本必须一致，当前网络协议版本为 `14`。
...
maxDistance = 4096.0
minHeightAboveGroundWithoutDH = 25
syncIntervalTicks = 5
...
多人游戏中，服务端的 common 配置决定“哪些目标会被同步”以及四项 Billboard 策略和无 DH 高度阈值；这些策略随每份远距完整快照强制下发，客户端本地同名值不覆盖服务器。每位玩家客户端本地 common 配置中的 `removeTerrainFog` 仍只决定“该客户端是否剔除普通地形雾”。Forge common 文件本身不会自动覆盖客户端文件，强制效果来自版本 14 网络快照。

### 2.1 未开启 DH 时的最低离地高度（2026-09-04）

服务端 `remoteVehicleRendering.minHeightAboveGroundWithoutDH` 默认 `25`，接受整数 `-1..2147483647`，单位米（1 米 = 1 格）。客户端未安装 DH 或关闭 DH 官方 `renderingEnabled` 时，离地高度 **小于** 阈值的超视距载具不渲染；等于阈值仍可显示。`-1` 禁用该限制，`0` 允许所有非负离地高度。

该阈值随每份完整快照下发，客户端仅在维度、序号校验通过后采用，退出世界时清理；客户端本地 common 同名值不能覆盖服务器。离地高度沿用服务端 `MOTION_BLOCKING` 高度图样本的插值结果，不是世界 Y 坐标，也不在客户端查询未加载地形。过滤发生在候选投影、预算和 Billboard 预热之前，适用于所有远距模型/Billboard 路径，不影响原生追踪实体、服务端目标授权、快照数量及区块租约。

DH 开关每帧读取，支持游戏内关闭/开启后恢复过滤/显示。RVP `distantHorizons.compatMode=OFF` 仅关闭兼容层，不代表 DH 自身关闭；API 初始化前或不受支持版本无法读到官方开关时，沿用已有兼容层的“已安装则暂视为开启”状态，不把深度合成失败当成 DH 关闭。

本次扩展将网络协议从 `6` 升到 `7`，客户端和服务端必须同步更新。实机回归可在水平距离大于 512 格处分别放置离地 24、25、26 米的目标，验证 DH 未安装、官方渲染关闭/开启以及阈值 `-1`、`0`、自定义值；默认无 DH 时仅 24 米目标隐藏。
...
类型矩阵另提供 `walking_players` 观察者键；它只表示步行玩家，不会作为载具目标类型。
...
| `RADAR_DETECTED` | 乘坐载具的玩家按类型矩阵授权；步行玩家使用 `walking_players` 项，1024 格以内可见，超过 1024 格仍需有效雷达探测。 |
| `VEHICLE_OCCUPANTS` | 乘坐载具的玩家按观察载具类型矩阵决定目标类型；步行玩家按 `walking_players` 项决定目标类型。默认值。 |
...
- 服务端权威的两个 Billboard 开关、图像来源、动态快照预热方式和无 DH 最低离地高度。
...
### 6.3 与本体雷达广播克隆的区别（2026-09-24）

本节前述代理属于 **RVP 超远距视觉快照链**，不能与本体
`ServerBroadcastEntities -> LocalVehiclePlayer.serverEntities` 的雷达广播克隆混为一套状态。
两者虽然默认都可能以 5 Tick 为同步周期，但所有权、用途和插值位置不同：

| 项目 | RVP 超远距视觉代理 | 本体雷达广播克隆 |
|---|---|---|
| 数据入口 | RVP 完整远距快照 | 本体 `ServerBroadcastEntities` |
| 客户端容器 | RVP 远距代理状态 | `LocalVehiclePlayer.serverEntities` |
| 是否加入 `ClientLevel` | 否 | 否 |
| 主要用途 | 远距载具主体模型 / Billboard 显示 | 本体雷达发现、锁定及相关 HUD 目标实体 |
| 原生运动平滑 | 双样本插值 + 有限外推 | 无；收包时 `xo/yo/zo` 与当前位置同时写成新样本，载具 `remoteTick()` 不推进位置 |
| 周期来源 | `remoteVehicleRendering.syncIntervalTicks` | 本体 `serverBroadcastEntitiesInterval`（默认 5 Tick） |

本体硬锁双绿色实心框原先直接读取广播克隆的包围盒中心；即使调用
`Mth.lerp(partialTick, xo, x)`，也会因为本体收包后 `xo == x` 而退化为常量。因此目标锚点在
两个包之间保持不动，到下一包再整体跳到新位置，形成 5 Tick 阶梯。

RVP 自 2026-09-24 起由 `RVP_ClientBroadcastVehicleInterpolator` 单独维护 HUD 显示轨迹：

- 只有同时满足 `AbstractVehicle`、`remote=true`、实体 ID 与对象身份都存在于
  `LocalVehiclePlayer.serverEntities` 的目标才进入跨包插值；普通世界实体继续使用 Minecraft
  的单 Tick `xo/yo/zo` 插值。
- 首次见到广播克隆时以当前包围盒中心初始化，不从原点或旧 ID 的历史位置飞入。
- 新广播到达时，先计算旧过渡在该 Tick 的连续位置，再以该位置为起点，在本体配置的一个
  广播周期内线性过渡到新样本。默认会引入约 5 Tick（20 TPS 下约 250 ms）的显示缓冲，换取
  帧间连续移动；这不是速度外推。
- 缓存以实体 ID 索引，同时保存实体对象身份；ID 复用会重建轨迹。离开载具、退出或切换世界、
  以及本体从 `serverEntities` 淘汰克隆时清理对应状态。
- 插值结果只提供给 RVP 的雷达硬锁框、TWS 单缺口扫描框、雷达显示器航迹点、可解析的外置雷达框
  及同目标导引头圈，不回写
  实体位置、速度、探测记录或锁定状态，因此不会改变雷达发现、STT、制导、碰撞和服务端权威状态。

这项 HUD 平滑也不替代本节 6.2 的远距模型插值：一套负责世界中的远距载具视觉代理，另一套只
负责本体雷达广播克隆在 RVP HUD 上的屏幕锚点。排查“模型平滑但锁定框跳”或“锁定框平滑但模型
跳”时，应分别检查两条数据链，不能只调其中一个同步周期。
...
### 7.2 主体模型与方案 A 部件状态绘制

远端通道仍只绘制载具主体，但基础高模路径会消费以下服务端权威部件状态：

- 全部 `WeaponUnit` 的局部俯仰角和偏航角，包括炮塔、炮管和武器站；角度与整车姿态一样使用双样本最短角差插值。
- `LandingGearUnit`、`DoorUnit`、`WeaponBayUnit` 以及 RVP 发射架开关部件的布尔终态。
- `LauncherDeployRuntimeManager` 的阶段、进度 Tick、当前俯仰和采样速度；俯仰部件同时走通用旋转状态。
- 开关终态改变时重建非世界代理，并在显示实例创建前写入状态，因此直接显示终态，不补播远距开关动画和声音。
...
- 独立 LOD、动态快照 Billboard 与槽位贴图仍为静态，不消费上述逐车部件状态；只有回退到基础高模或观瞄策略选择基础高模时可见。
...
| `remoteVehicleRendering.minHeightAboveGroundWithoutDH` | 整数 -1～2147483647 | `25` | 服务端下发；未安装 DH 或关闭其地形渲染时，隐藏低于阈值（米/格）的超视距载具；-1 禁用，等于阈值仍显示。 |
...
walking_players = ["helicopter", "aircraft"]
```

默认情况下，空中观察者可看所有载具，地面载具和步行玩家观察者只看空中目标。矩阵在 `VEHICLE_OCCUPANTS`、`RADAR_DETECTED` 和乘坐载具的 `ALL_PLAYERS` 观察者上生效；`ALL_PLAYERS` 下步行玩家仍可看所有目标。
...
| `remoteVehicleRendering.distantHorizons.compatMode` | enum | `AUTO` | `OFF` 关闭适配；`RVP_FIRST` 保证完整载具显示在 DH 地形之上；`AUTO` 优先深度感知并允许降级；`DEPTH_AWARE` 失败时隐藏。 |
...
| `remoteVehicleRendering.distantHorizons.protectTrackedVehicles` | boolean | `true` | 是否让客户端真实载具进入 DH 颜色/深度保护层；不改变服务端追踪。 |
| `remoteVehicleRendering.distantHorizons.trackedOcclusionBiasBlocks` | 0～2 | `0.5` | 真实载具保护层的独立 DH 遮挡容差，单位格。 |
| `remoteVehicleRendering.distantHorizons.maxProtectedTrackedVehicles` | 1～256 | `64` | 每帧真实载具保护上限，按屏幕贡献、距离和实体 ID 稳定选择。 |
...
安装 Distant Horizons 且客户端配置不是 `OFF` 时，远距代理与客户端真实载具都会在 `AFTER_SKY` 建立独立计划，然后等待 DH 官方 `BeforeApplyShader` API 事件。兼容层先用动态远距投影绘制并合成代理，再复制已更新的 DH 深度，用 Minecraft 当帧投影绘制真实载具只读保护副本。每层分别用自己的逆投影和 DH 逆投影恢复相机视深度，从而覆盖“512 格内真实载具被 DH 后续颜色或深度覆盖”的断层。

真实载具保护副本不会再次调用完整 `EntityRenderer`，只复制正常模型实例已经提交的姿态并绘制主体、脱落骨骼、special bones 和只读武器挂载几何；不会推进动画、粒子、声音或 `lastRenderTime`。第一人称座舱/瞄准镜排除本机外壳，第三人称仍启用保护。
...
2. 客户端与服务端 mod 版本是否一致，网络协议是否均为版本 14。
3. 目标水平距离是否严格大于 512 且不超过 `maxDistance`。
4. `VEHICLE_OCCUPANTS`/`RADAR_DETECTED` 下玩家的步行状态或所乘载具类型是否对应正确的矩阵项。
5. 观察载具类型矩阵是否允许目标类型。
...
- 确认服务端四项 Billboard 配置已经随版本 14 快照下发；多人服客户端本地同名配置不会覆盖服务器。
...
| `S2CRemoteVehicleVisualSnapshot` | 协议版本 14 的完整远端载具快照、类型化部件状态与服务端 Billboard 策略。 |
...
| `RVP_RemoteVehicleFramePlan` / `RVP_DhVehicleFramePlan` / `RVP_DhVehicleFrameCoordinator` | 远距计划、remote/tracked 统一信封及 DH/原有/晚期降级的唯一消费路由。 |
