...
自动化测试覆盖了投影数学、Forward-Z/Reverse-Z、DH 矩阵到 JOML 的逐字段映射、帧计划唯一消费、强制非扩展投影的渲染作用域，以及“只有隔离桥可引用 DH 类型”的架构约束。当前完整 Gradle 构建曾通过 370 项测试，发布 jar 审计未包含 `com/seibel/distanthorizons` 类。
