...
兼容层运行时要求 DH API 7.1 或更高，并校验原生 OpenGL 渲染能力。API 版本不足、非原生 OpenGL、纹理不可用、投影端点无法判定或 FBO 不完整时会按配置明确降级。默认 `AUTO + SILHOUETTE` 在世界末端只合成青色边缘；`ALWAYS_VISIBLE` 会忽略地形深度并产生穿山语义，必须由用户明确启用。延迟透明/Oculus 默认标记为 `DEFERRED_SHADER_UNVERIFIED` 并降级，只有显式打开实验选项才尝试 apply 前通道，不能视为正式光影支持。
...
| `client.compat.distanthorizons` | DH API 7.1 类加载隔离、投影验证、离屏资源、深度复制/合成、显式降级和诊断。 |
