...
| chaffResistance       | 导引头对箔条目标的锁定抗性 0~1：ARH/AIR 开启导引头后**可锁箔条**（扫描候选含 CHAFF 实体），但按此值对箔条施加评分罚分（越大优先级越低，非完全不可锁）。`0` = 无惩罚 | float   | 0      |
...
| rvpMax | JSON 键 `rvp_maxg`。RVP 最大法向过载，单位 G；显式配置后实体与虚拟制导均调用 `applySteering`，并优先于 `turning_factor`。0 表示禁止转向 | Double | null |
