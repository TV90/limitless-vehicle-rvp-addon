...
| `dust_ring_start_tick` / `dust_ring_full_tick` | 贴地尘环的绝对开始时刻，以及到达 `dust_radius_factor` 配置半径并立即开始消散的时刻（tick）；均接受非负整数、不设业务上限。尘环从开始到 full tick 匀速扩张，之后以相同径向速度继续外扩并线性变淡；消散时长自动等于成形时长，结束时半径为 full tick 半径的两倍。地面高度按爆心中心 X/Z 的碰撞地表采样；爆心离地高度不超过 20 格时允许生成，超过 20 格时直接跳过尘环地表采样、环段创建和绘制。20 格阈值由代码字段 `RVP_ThermobaricEffectInstance.MAX_DUST_RING_AIRBORNE_HEIGHT`（`20.0F`）控制，不属于 `preset_data` JSON schema；客户端暂时无法识别地面时保留原行为。 | `10` / `25`                                                       |
...
| `blind_hit_count` | 激光致盲触发所需命中次数，**默认 `0` = 关闭致盲**（仅显式配置 >0 的激光生效）。命中玩家/gunner/其骑乘载具时累计，窗口内攒满触发致盲，触发后计数清零重新累计。 |
| `blind_hit_window_tick` | 致盲命中累计窗口（tick），超窗清零重计。默认 `100`。 |
| `blind_duration_tick` | 致盲时长（tick）；`0` = 关闭。玩家=白色闪光滤镜（S2CLaserBlind），gunner=失去目标（期间仅雷达制导/雷达中段的 SARH/ARH/AIR 导弹可用且要求载具保有可用雷达）。默认 `200`。 |
...
| `hitbox_damage_factor_default` | 未命中任何已配置骨骼时的默认受击倍率。**仅作用于弹体直击伤害**（爆炸伤害不走此参数）。 | `1.0` |
| `hitbox_damage_factor` | 对象：`骨骼名 → 倍率`。直击命中该骨骼时使用对应倍率。 | 空对象 |
...
>
> **ERA 入射角上界（2026-09-20）**：直击命中挂有激活 ERA 的骨骼时，直击伤害衰减（`damage_decay` 的 `angle` 分段）所用入射角被钳到 `min(实际入射角, eraMaxIncidenceAngle)`（common 配置 `explosion.eraMaxIncidenceAngle`，默认 35 度，90 关闭）——模拟 ERA 对大角度入射的等效防护；跳弹判定（`bounce_incidence_angle`）使用原始角度不受影响。
...
`rvp_rf` 下当前选中武器为RVP `missile` 时也可切换完整三态：`STABLE` 严格跟随雷达硬锁目标中心；`SEMI_AUTO` 在硬锁目标方向上叠加玩家锁存的局部俯仰/方位偏置；`OFF` 完全手动。只认本车火控雷达或外置雷达的正式硬锁，不认仅写入 `WeaponUnit.lockedEntity` 的TWS/导引头软航迹；无硬锁时不驱动自动视线。当前制导阶段为 `SACLOS` 且已有 `predict_target_pos: true` 时，`STABLE` 还会让服务端以正式硬锁目标生成临时实体意图，复用现有PIP预测拦截；切换到其它模式、丢锁、请求超时或PIP门控不满足时立即回退原SACLOS视线制导。临时意图不写弹体持久 `targetEntity`。该行为不新增JSON字段。
