...
| `explosion_damage_factor_default` | 爆炸伤害的默认倍率（2026-09-20 拆分新增）。**仅作用于爆炸伤害**（本体武器与 RVP 弹体的爆炸，对载具目标）；未配置时爆炸不缩放。 | `1.0` |
| `explosion_damage_factor` | 对象：`骨骼名 → 倍率`（2026-09-20 拆分新增）。爆炸伤害按「爆心 → 载具中心」线段命中的最近骨骼取对应倍率（天然命中面向爆心的装甲面）。**与直击倍率完全独立**，未配置不缩放。 | 空对象 |
