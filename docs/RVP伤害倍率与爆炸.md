...
本文说明 `damage_model_data.damage_factor` 的配置方式，以及 RVP 如何对**直击**与**爆炸波及**伤害应用倍率。
...
| 爆炸 | 在 `onImpact` 中乘系数 | `RVP_Explosion` + 本体 `VehicleExplosion` |
...
| 爆炸波及 | `RVP_Explosion` → 本体 `VehicleExplosion#hurt`（经 Mixin） |

**不缩放**：`detonate_data.explosion_data` 中的爆炸威力仅作为爆炸计算的输入；波及伤害在 `hurt` 阶段乘倍率。方块破坏、粒子、音效逻辑与本体一致。

未配置 `damage_factor` 或全部为默认 1 时，行为与未引入 RVP 倍率前相同。
...
org.ywzj.rvp.util.RVP_Explosion               # 继承 VehicleExplosion
org.ywzj.rvp.util.RVP_ExplosionDamageContext  # ThreadLocal，供 Mixin 读取
org.ywzj.rvp.mixin.VehicleExplosionMixin      # 缩放爆炸波及伤害
```

RVP 弹体爆炸请使用 `RVP_Explosion`，不要直接 `new VehicleExplosion(...)`，否则不会应用 `damage_factor`。
...
1. 对玩家 / 僵尸 / 旋翼机各打一发，观察伤害是否符合倍率。
2. 仅改 `vehicles` 中一项，确认 `vehicle_default` 与单独项优先级。
3. 爆炸弹：对比直击与爆炸边缘伤害是否都乘倍率。
