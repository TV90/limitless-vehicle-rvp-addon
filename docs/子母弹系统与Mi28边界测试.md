...
| `explosion_after_release` | 本方案完成释放后立即执行母弹完整引爆链，再移除母弹；可用于开舱同时引爆母弹战斗部 |

`explosion_after_release` 使用母弹自身的 `detonate_data` 与 `explosion_data`。如果母弹未配置可生效的爆炸或落点效果，仍会在释放后移除，但不会凭空生成爆炸伤害。该动作引发的母弹引爆不会再次触发 `on_fuse` 释放方案，避免重复释放或递归引爆。
...
| 母弹不爆 | `parent_action: discard_after_release` 会直接丢弃；需要释放后引爆时改为 `explosion_after_release`，并确认母弹已配置 `detonate_data.explosion_data.explode: true` |
