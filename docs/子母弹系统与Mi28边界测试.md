...
- 对会生成散布 Y 的模式（如 `stratified_cone`），子体可用 `deployment_vertical_half_life_ticks` 单独衰减初始纵向分量；重力与显式 `payloads_velocity[1]` 不受影响。
- `stratified_cone` 可配置 `cone_azimuth_mode: spawn_radial`，让圆锥 X/Z 方位使用最终出生点相对释放中心的水平外向方向，同时保留按子体序号分层的向下 Y 分量；水平偏移退化时稳定回退黄金角。默认 `golden_angle` 不改变旧武器。
