...
> 文档日期：2026-09-14  
> 状态：实施中；阶段 A、B、C、D 已完成，阶段 E～G 尚未实施
...
当前阶段 B 的本车雷达适配器还承担目标失效兜底：当索敌目标为空或死亡时，清除当前武器站全部 `RadarUnit` 锁及 root `WeaponUnit` 锁，防止目标 RWR 或 SARH 中继残留。有效目标的范围、射界、箔条门控及锁定流程不变。
...
> 2026-09-16 补充：将 `d3f765dd` 的目标丢失清锁修复接入 `RVP_GunnerRadarActions`，并加入阶段 B 基线断言；详见进度交接文档。
...
> 实施状态：已于 2026-09-26 完成；固定 BehaviorPlan、内建行为目录、旧权威路径删除与验证结果见
> [RVP_Gunner重构进度交接_20260914.md](./RVP_Gunner重构进度交接_20260914.md)。
