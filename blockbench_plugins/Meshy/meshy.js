(function() {
    ...
    const pluginSettings = [
        ...
        {
            id: 'meshy_smooth_mesh_normals',
            name: 'Smooth Mesh Normals',
            description: 'Averages adjacent face normals using a smoothing angle threshold',
            category: 'export',
            value: true,
            plugin: pluginInfo.id,
        },
        {
            id: 'meshy_smooth_angle_deg',
            name: 'Smoothing Angle (deg)',
            description: 'Faces with a larger angle than this will not be averaged together',
            category: 'export',
            type: 'number',
            min: 0,
            max: 180,
    ...
    function compileMesh(polyMesh, mesh) {
        ...
        const smoothEnabled = settings["meshy_smooth_mesh_normals"]?.value ?? true;
        const smoothAngleDeg = Math.max(0, Math.min(180, Number(settings["meshy_smooth_angle_deg"]?.value ?? 60)));
        const smoothCos = Math.cos(smoothAngleDeg * Math.PI / 180);
    
        //Make a map of faces a vertex is appart of 
        for (let faceKey in mesh.faces) {
            let face = mesh.faces[faceKey];
            faceNormalMap.set(faceKey, normalize3(face.getNormal()));
        ...
        for (let faceKey in mesh.faces) {
            ...
            const faceNormal = faceNormalMap.get(faceKey) ?? normalize3(face.getNormal());
            const poly = face.getSortedVertices().map((vertexKey) => {
                ...
                const n = getVertexNormalForFace(mesh, vertexKey, faceNormal, vertexFacesMap, faceNormalMap, smoothEnabled, smoothCos);
                const nk = normalKey(n);
                const nIndex = normals.get(nk) ?? (() => {
                    const index = polyMesh.normals.length;
                    normals.set(nk, index);
                    polyMesh.normals.push(n);
                    return index;
                })();

                return [postionMap.get(vertexKey), nIndex, uIndex];
            });
            if (poly.length < 4) 
                //Fill the poly with the first vertex if less than a quad change: Support for less than 3 vertices
                polys.push(poly.concat(Array(4 - poly.length).fill(poly[0])));
            else
                polys.push(poly);
        }
    ...
    function getVertexNormalForFace(mesh, vertexKey, baseFaceNormal, vertexFacesMap, faceNormalMap, smoothEnabled, smoothCos) {
        if (settings["meshy_skip_mesh_normals"].value) return [ 0,1,0 ];
        if (!smoothEnabled) {
            return getVertexNormal(mesh, vertexKey, vertexFacesMap);
        }
        let sum = [0, 0, 0];
        let count = 0;
        const faces = vertexFacesMap.get(vertexKey) || [];
        for (let fk of faces) {
            const fn = faceNormalMap.get(fk) ?? normalize3(mesh.faces[fk]?.getNormal?.() ?? [0, 1, 0]);
            const d = clamp01(dot3(baseFaceNormal, fn));
            if (d < smoothCos) continue;
            sum[0] += fn[0];
            sum[1] += fn[1];
            sum[2] += fn[2];
            count++;
